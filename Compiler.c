...
static int variable()
{
    ...
    if (!is_identifier(token)) {
        ERROR("Expected variable, found '%c'\n", token);
        exit(EXIT_FAILURE);
    }

    reg = next_register();
    CodeGen(LOAD, reg, token, EMPTY_FIELD); 							// LOAD Rx <id>
    next_token();
    return reg;
}
...
static int expr()
{
	...
	case '-':
        ...
        right_reg = expr();
        reg = next_register();
        CodeGen(SUB, reg, left_reg, right_reg); 							// SUB Rx Ry Rz
        return reg;
    ...
    case '*':
        ...
        right_reg = expr();
        reg = next_register();
        CodeGen(MUL, reg, left_reg, right_reg); 							// MUL Rx Ry Rz
        return reg;
    ...
    case '&':
        ...
        right_reg = expr();
        reg = next_register();
        CodeGen(AND, reg, left_reg, right_reg); 							// AND Rx Ry Rz
        return reg;
    ...
    case '|':
        ...
        right_reg = expr();
        reg = next_register();
        CodeGen(OR, reg, left_reg, right_reg); 							// OR Rx Ry Rz
        return reg;
...
static void assign()
{
    ...
    // Parse the variable on the left side
    var = token;
    next_token();
    ...
    // Generate STORE instruction to save the result back to the variable
    CodeGen(STORE, var, expr_reg, EMPTY_FIELD); // STORE <id> Rx
}
...
static void read()
{
    ...
    // Parse the variable
    if (!is_identifier(token)) {
        ERROR("Expected variable after '?', found '%c'\n", token);
        exit(EXIT_FAILURE);
    }
    var = token;
    next_token();
    ...
    CodeGen(READ, var, EMPTY_FIELD, EMPTY_FIELD); // READ <id>
}
...
static void print()
{
    ...
    CodeGen(WRITE, var, EMPTY_FIELD, EMPTY_FIELD); // WRITE <id>
}
...
static void stmt()
{
    ...
    case 'd': case 'e': case 'f':
        ...
        break;
    default:
        ERROR("Unknown statement start '%c'\n", token);
        exit(EXIT_FAILURE);
    }
}
